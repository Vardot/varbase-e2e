...
30 steps, defined in `tests/step-definitions/a11y.steps.js`.
...
| 22 | `Then the page should have no accessibility violations` |
| 23 | `Then the element "main" should not violate the accessibility rule "color-contrast"` |
| 24 | `Then the element "main" should pass an accessibility audit` |
| 25 | `Then the page should pass an accessibility audit excluding "iframe.payment"` |
| 26 | `Then the page should not violate the accessibility rule "color-contrast"` |
| 27 | `Then I print accessibility violations` |
| 28 | `Then the page should pass the accessibility rules "image-alt, label"` |
| 29 | `Then the page should pass the full accessibility check` |
| 30 | `Then I print the full accessibility check` |
...
```js
/^the element "([^"]*)" should not violate the accessibility rule "([^"]*)"$/
```

**Examples**

```gherkin
Then the element "main" should not violate the accessibility rule "color-contrast"
Then the element "#checkout" should not violate the accessibility rule "label"
And the element ".card" should not violate the accessibility rule "heading-order"
Then the element "footer" should not violate the accessibility rule "link-name"
Then the element "[data-testid=nav]" should not violate the accessibility rule "landmark-unique"
```
...
```js
/^the page should pass the full accessibility check(?: at level "(A|AA|AAA)")?$/
```
...
```js
/^(?:I |we )*print the full accessibility check$/
```
...
When I go to "/about-us"
  Then I print the full accessibility check
And we print the full accessibility check
Given I am an anonymous user
  Then I print the full accessibility check
Then I print the full accessibility check
  And the page should have no critical accessibility violations
